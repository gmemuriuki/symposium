...
If private state is absent while daily files survive, Symposium initializes the latest-opened-day high-water mark and identifier-window anchor to the later of the current UTC day and the newest day named by a canonical `events-YYYY-MM-DD.jsonl` or `metrics-YYYY-MM-DD.jsonl` file. Other names are ignored. Malformed state and a syntactically valid unsupported state version are not treated as absence and are never replaced by this initialization path.
